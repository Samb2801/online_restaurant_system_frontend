 ...
 const  getLogOut= () =>{
  if(window.confirm("Are you sure that you want to log off?")){ 
    ...
    <Link className="navbar-brand" to="/">
      ...
      <span className="nav-title">Samart</span><span className="nav-title-to">Restro</span>
    </Link>
    ...
    {totalItems!==0 ? totalItems: null}</span></i></div>
          <div className="px-5 pt-2 me-15 dropdown">
            ...
            <ul id="dn-menu" className="dropdown-menu">
    <li><Link id = "edit" className="dropdown-item" to="/editprofile" title="Edit Profile">Edit Profile</Link></li>
    <li id = "logout" className="dropdown-item" onClick={getLogOut} role="button" title="Log Out">Log Out</li>
    </ul>
            </div>
      {/* </div> */}
